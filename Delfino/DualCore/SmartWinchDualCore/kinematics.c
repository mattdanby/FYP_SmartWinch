...
float motax, motay, motbx, motby ,motcx, motcy ,motdx, motdy;
...
struct XYZ_coord_struct tenandsag2coord(float tena,float tenb,float tenc,float tend,float slena,float slenb,float slenc,float slend) {
    //Converts the four tensions from the strain gauge and the sagged cable...
    //lengths to the coordinates of the payload
    float X,Y,Z,L1,L2,L3,L4,D1,D2,D3,D4,tenah,tenbh,tench,tendh,va,vb,vc,vd,ka,kb,kc,kd,t1,t2,t3,t4,r1,r2,r3,r4,dif1,dif2,dif3,dif4,maxans,xoff,yoff,tot,tot2;
...
struct length4_struct XYZ_to_length4(float X, float Y, float Z) {
	//Int X,Y and Z are the payload coordinates
	
	
	struct length4_struct lengths;
	lengths.lengtha = sqrt((pow(X-motax,2))+(pow(Y,2))+(pow(Z,2)));
	lengths.lengthb = sqrt((pow(X-motbx,2))+(pow((motby-Y),2))+(pow(Z,2)));
	lengths.lengthd = sqrt((pow((motdx-X),2))+(pow(Y,2))+(pow(Z,2)));
	lengths.lengthc = sqrt((pow((motcx-X),2))+(pow((motcy-Y),2))+(pow(Z,2)));
...
struct length4_struct coord2ten_sag(float x, float y, float z,float comb_uplift) {
	...
	struct length4_struct strght_len = XYZ_to_length4(x,y,z);
	...
	struct length4_struct len_final = {len_out_op[(4-remove_cable)%4],len_out_op[(5-remove_cable)%4],len_out_op[(6-remove_cable)%4],len_out_op[(7-remove_cable)%4],1};
...
void set_motor_coord(float ab, float cd, float ad, float ac, float bd) {
    ...
    if (motbxt<0) {
        ...
        motax = fabs(motbxt);
        motbx = 0;
        motcx = motcxt+fabs(motbxt);
        motdx = motdxt+fabs(motbxt);
    } else {
        motax = 0;
        motbx = motbxt;
        motcx = motcxt;
...
struct input_lengths mot_coord2sides(float max,float may,float mbx,float mby,float mcx,float mcy,float mdx,float mdy) {
